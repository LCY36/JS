...
const getAllDays = year => {
  ...
  while (lastDayInArray.getTime() !== lastDay.getTime()) {if (window.CP.shouldStopExecution(0)) break;
...
dates.forEach(date => {
  ...
  if (date.getDate() === 1 && date.getDay() !== 0) {
    for (let i = 0; i < date.getDay(); i++) {if (window.CP.shouldStopExecution(1)) break;
